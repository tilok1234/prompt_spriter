...
const stagingDir = "C:/Users/headc/Documents/prompt_spriter/workspace/staging/enemy-mob-32-sapspitter-gecko";
...
writeFileSync("tools/build-sapspitter-gecko.lua", luaScript);
console.log("Written tools/build-sapspitter-gecko.lua");

// Write submission.json
const promptText = `Read and follow the project documentation.

Promptinator entry ID: prompt-0009-sapspitter-gecko
Prompt formula: structured-v1

Create an enemy-mob-32 sprite named "Sapspitter Gecko".

## Creative brief

- Collection: Thornwood Brood. Woodland creatures shaped by roots, thorns, mushrooms, and ancient forest magic.
- Core concept: A mobile area-denial creature that covers escape routes with sticky pools.
- Body and silhouette: Low elongated body, spread toes, curled tail, and oversized throat.
- Signature features: Amber throat sac, leaf-shaped toes, and translucent sap droplets.
- Palette and materials: Lime green, bark brown, honey amber, and glossy wet sap.
- Movement personality: Skittish, twitchy, and quick to retreat.
- Attack concept: Spits arcing sap globules that burst into slowing puddles and smaller droplets.
- Directional details: The tail curls clockwise when viewed from above, and the right eye is darker.
- Avoid: Chameleon features, poisonous acid interpretation, oversized cartoon tongue.
...
const submission = {
  ...
  schemaVersion: "1.0.0",
  jobId: "enemy-mob-32-sapspitter-gecko",
  assetId: "enemy-mob-32-sapspitter-gecko",
  baseRevisionId: null,
  requestedName: "Sapspitter Gecko",
  request: promptText,
  category: {
    id: "enemy-mob-32",
    version: "0.1.0"
...
// Execute Aseprite script
const asepritePath = "C:\\Program Files\\Aseprite\\Aseprite.exe";
console.log("Executing Aseprite script...");
execSync(`"${asepritePath}" -b -script tools/build-sapspitter-gecko.lua`, { stdio: "inherit" });
console.log("Aseprite execution complete.");

const valResult = validateSubmissionDirectory({
  jobDirectory: resolve(repositoryRoot, "workspace/staging/enemy-mob-32-sapspitter-gecko"),
...
if (valResult.warnings.length > 0) {
  checks.push({
    ...
    level: "warning",
    status: "warning",
    message: valResult.warnings.join("; "),
  });
} else {
  checks.push({
    code: "structural-validation",
    level: "info",
    status: "passed",
    message: "Submission validated cleanly with no errors or warnings.",
  });
}
...
const validationReport = {
  ...
  schemaVersion: "1.0.0",
  jobId: "enemy-mob-32-sapspitter-gecko",
  status: valResult.computedStatus,
  checks,
  createdAt: new Date().toISOString()
};

writeFileSync(`${stagingDir}/validation.json`, JSON.stringify(validationReport, null, 2));
...
writeFileSync(`${stagingDir}/completion.json`, JSON.stringify(completionMarker, null, 2));
