...
const stagingDir = "C:/Users/headc/Documents/prompt_spriter/workspace/staging/enemy-mob-32-mudskipper-skirmisher";
mkdirSync(stagingDir, { recursive: true });

// Remove old validation.json and completion.json if present to start fresh preflight
if (existsSync(`${stagingDir}/validation.json`)) unlinkSync(`${stagingDir}/validation.json`);
if (existsSync(`${stagingDir}/completion.json`)) unlinkSync(`${stagingDir}/completion.json`);
...
writeFileSync("tools/build-mudskipper-skirmisher.lua", luaScript);
console.log("Written tools/build-mudskipper-skirmisher.lua");

// 2. Execute Aseprite script
const asepritePath = "C:\\Program Files\\Aseprite\\Aseprite.exe";
console.log("Running Aseprite CLI...");
execSync(`"${asepritePath}" -b -script tools/build-mudskipper-skirmisher.lua`, { stdio: "inherit" });

// 3. Write submission.json
const promptText = `Read and follow the project documentation.

Promptinator entry ID: prompt-0013-mudskipper-skirmisher
Prompt formula: structured-v1

Create an enemy-mob-32 sprite named "Mudskipper Skirmisher".

## Creative brief

- Collection: Mireborn Swarm. Swamp creatures built around mud, reeds, bubbles, toxins, shallow water, and deceptive movement.
- Core concept: A fast amphibious attacker that zigzags around the player.
- Body and silhouette: Low fishlike body with muscular front fins and a tall dorsal crest.
- Signature features: Bulging eyes, muddy fin-hands, and bright throat markings.
- Palette and materials: Blue-gray, mud brown, orange, and slick wet skin.
- Movement personality: Hyperactive, erratic, and confrontational.
- Attack concept: Performs short sideways hops while firing alternating left-right volleys of water darts.
- Directional details: The dorsal crest bends toward the creature’s right, and one eye is scarred.
- Avoid: Realistic small mudskipper, merfolk body, overly comical expression.
...
const submission = {
  ...
  schemaVersion: "1.0.0",
  jobId: "enemy-mob-32-mudskipper-skirmisher",
  assetId: "enemy-mob-32-mudskipper-skirmisher",
  baseRevisionId: null,
  requestedName: "Mudskipper Skirmisher",
  request: promptText,
  category: {
    id: "enemy-mob-32",
    version: "0.1.0"
...
// 4. Perform preflight validation to determine exact computedStatus & warnings
console.log("Performing submission preflight validation...");
const preflight = validateSubmissionDirectory({ jobDirectory: stagingDir });
console.log(`Preflight computed status: ${preflight.computedStatus}`);
...
const validation = {
  ...
  schemaVersion: "1.0.0",
  jobId: "enemy-mob-32-mudskipper-skirmisher",
  status: preflight.computedStatus,
  checks: [
    {
      code: "structural-validation",
      level: preflight.computedStatus === "passed" ? "info" : "warning",
      status: preflight.computedStatus === "passed" ? "passed" : "warning",
      message: preflight.technicalWarnings.length > 0
        ? `Submission validated with advisory warnings: ${preflight.technicalWarnings.join("; ")}`
        : "Submission structural validation passed clean."
    }
  ],
  createdAt: now
};
...
// 5. Final completion-required validation check
console.log("Running final completion-required validation...");
execSync("npm.cmd run validate:submission -- workspace/staging/enemy-mob-32-mudskipper-skirmisher --require-completion", { stdio: "inherit" });
