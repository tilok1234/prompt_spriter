...
2. The user requests a named sprite or asks for the next Promptinator sprite.
   ...
   documented trusted command; the user does not copy a prompt. When an
   explicit v2 test batch is active, the claim is constrained to its selected
   Ready entries and refuses to spill into the normal queue after exhaustion.
...
Intake may accumulate across sessions and is the sole active candidate lane.
It is not a chat queue.
...
- **Approve:** set the exact candidate revision as the Library revision, but
  only when it has no unresolved revision notes.
- **Add revision note:** keep the candidate in Intake and mark it **Revision
  requested**.
- **Deny:** move the candidate into the dormant Denied lane.

All three actions require an explicit confirmation in the viewer. A revision
note must be non-empty and may optionally target a direction, animation, and
one or more one-based frame numbers. More notes can be added while the
candidate remains in Intake. Unresolved notes are the approval safeguard:
Approve is rejected until trusted revision ingestion resolves all of them.
...
For an unapproved candidate, **Add revision note** creates the first actionable
note without moving lanes. For an approved asset with no active candidate,
**Start revision** creates a separate Intake candidate against the exact
approved revision and requires the first actionable note.

1. The user reviews one or more Intake items and writes actionable notes.
...
match the exact active Intake candidate and requires at least one unresolved
note. It also requires the name, category, and style to match the existing
asset. A stale or mismatched job is refused instead of being attached to the
wrong history.
...
1. In Intake, select one or more exact candidates marked **Revision requested**.
...
3. Prompt Spriter rechecks every selected revision, its current
   `review.updatedAt`, its Intake lane, and its unresolved notes.
4. It writes:
...
- Denied offers **Return to Intake** as a reversible recovery action.
- Restore always returns the candidate to Denied with notes and history.
- Archiving a newer working candidate does not remove an older approved Library
  revision.
- Agents cannot archive candidates.

The Archive control is shown only for a candidate currently in Denied. The
Archive view offers Restore, which returns the same candidate and preserved
notes to Denied. Returning that candidate to active work is a separate,
explicit action.
...
- **Start revision** requires a note and opens the exact approved revision as a
  separate working candidate in Intake;
